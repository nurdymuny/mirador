...
// ─── Davis Field Equations engine ───────────────────────────────────────────

const tau = (d) => Math.log10(d.auc24 / d.ic50);
const kBarrier = (R) => R <= 0.001 ? 999.0 : Math.max(1 / R - 1, -1.0);
const kPathway = (d, rKey) => d.kAdmet + kBarrier(d.pen[rKey] || 0.3);
const cSite = (d, rKey) => { const k = kPathway(d, rKey); return tau(d) / Math.max(k, 0.01); };

function cComboActive(drugs, rKey) {
  let tG = 0, wT = 0;
  for (const d of drugs) {
    const k = Math.max(kPathway(d, rKey), 0.01);
    const g = 1 / k; tG += g; wT += tau(d) * g;
  }
  if (tG <= 0) return 0;
  return (wT / tG) * tG; // tau_combo * total_conductance
}

const phiThreshold = (drugs, rKey, thresh = 1.0, fA = 1e-6) => {
  const cA = cComboActive(drugs, rKey);
  if (cA <= 0) return Infinity;
  return Math.max((thresh / cA - fA) / (1 - fA), 0);
};

const cWithLRA = (drugs, rKey, phi, fA = 1e-6) => {
  return (fA + phi * (1 - fA)) * cComboActive(drugs, rKey);
};

// ─── Sidebar content per stage ──────────────────────────────────────────────

function SidebarContent({ stage, activeDrugs, resData, lraIdx, clearOrder, sVal, mob, expanded, FONT }) {
  ...
  if (stage === 0) {
    const maxTau = Math.max(...DRUGS.map(d => tau(d)));
    return (<>
      ...
      {expanded && (<>
        ...
        {activeDrugs.map(d => (
          <div key={d.key} style={{ fontSize: 9, color: "#64748b", marginBottom: 4, fontFamily: FONT }}>
            {d.abbr}: log₁₀({d.auc24.toLocaleString()}/{d.ic50}) = <span style={{ color: d.color, fontWeight: 600 }}>{tau(d).toFixed(4)}</span>
          </div>
        ))}
      </>)}
      <Divider />
    ...
    return (<>
      ...
      {expanded && (<>
        ...
        {resData.map(r => {
          ...
          const cLRA = lraIdx !== null ? cWithLRA(activeDrugs, r.key, LRAS[lraIdx].phi) : cNoLRA;
          return (
...
export default function HivApp() {
  ...
  const resData = useMemo(() => {
    if (!activeDrugs.length) return RESERVOIRS.map(r => ({ ...r, cA: 0, cTotal: 0, phiNeeded: Infinity, drugRank: [], bottleneck: "Geometric" }));
    return RESERVOIRS.map(r => {
      const cA = cComboActive(activeDrugs, r.key);
      const cTotal = 1e-6 * cA;
      const phiN = phiThreshold(activeDrugs, r.key);
      const bestPhi = lraIdx !== null ? LRAS[lraIdx].phi : 0.015;
      const drugRank = activeDrugs.map(d => ({
        abbr: d.abbr, color: d.color, R: d.pen[r.key] || 0.3,
        kB: kBarrier(d.pen[r.key] || 0.3), c: cSite(d, r.key),
      })).sort((a, b) => b.c - a.c);
      let bn = "Dynamic";
      if (cA < 1) bn = "Geometric";
      else if (bestPhi >= phiN) bn = "Cleared";
      return { ...r, cA, cTotal, phiNeeded: phiN, drugRank, bottleneck: bn };
    });
  }, [activeDrugs, lraIdx]);
  ...
  const clearOrder = useMemo(() =>
    [...resData].map(r => ({ name: r.label, score: r.cA / Math.max(r.frac, 0.001) }))
        ...
        <div style={{ flex: mob ? "1 1 100%" : (vizExpanded !== null ? "1 1 340px" : "1 1 520px"), minWidth: 0, width: "100%", transition: "flex 0.35s ease" }}>
          ...
          <StageCard stage={2} current={stage} title="THE BARRIERS" subtitle="K_barrier heatmap — tissue:plasma ratios" accent="#a855f7"
            ...
            {xCell && (() => {
              ...
              const kB = kBarrier(R);
              const t = tau(drug);
              const kTotal = drug.kAdmet + kB;
              const c = t / Math.max(kTotal, 0.01);
              return (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
                ...
                <tbody>
                  {resData.map(r => {
                    ...
                    const cLRA = lraIdx !== null ? cWithLRA(activeDrugs, r.key, LRAS[lraIdx].phi) : null;
                    return (
                      <React.Fragment key={r.key}>
          ...
          <StageCard stage={4} current={stage} title="THE REPORT" subtitle="Predictions, clearance ordering, and clinical implications" accent="#14b8a6" onJumpTo={() => setStage(4)}>
            ...
            {[
              ...
              { title: "Darunavir dominates CNS", detail: `DRV C_site=${resData.find(r => r.key === "CNS")?.drugRank.find(d => d.abbr === "DRV")?.c.toFixed(3) || "N/A"} — best single drug at CNS. Matches PI preference for HAND.`, color: "#a855f7", src: "Cusini 2013",
                math: `DRV: τ=${tau(DRUGS[3]).toFixed(2)}, R(CNS)=0.05, K_barrier=${kBarrier(0.05).toFixed(1)}. C = ${tau(DRUGS[3]).toFixed(2)} / ${(0.10 + kBarrier(0.05)).toFixed(2)} = ${cSite(DRUGS[3], "CNS").toFixed(3)}.` },
        ...
        {(() => {
          ...
          return (
            <div style={{
              ...
              <SidebarContent stage={stage} activeDrugs={activeDrugs} resData={resData} lraIdx={lraIdx} clearOrder={clearOrder} sVal={sVal} mob={mob} expanded={isExp} FONT={FONT} />
            </div>
          );
        })()}
