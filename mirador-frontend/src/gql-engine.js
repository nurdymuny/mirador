...
// ── Sample card definitions (tested in gql-engine.test.js) ─────────
export const NL_GROUPS = [
  {
    label: 'CLINICAL QUESTIONS', color: '#64b5f6',
    questions: [
      { q: "Which HIV drugs cross the blood-brain barrier?",
        gql: "COVER ON mirador_universe WHERE disease = 'hiv' AND tissue = 'cns' EVALUATE coherence RANK BY coherence DESC WITH CONFIDENCE, PROVENANCE",
      ...
      { q: "What kills MRSA inside bone tissue?",
        gql: "COVER ON mirador_universe WHERE disease = 'mrsa' AND tissue = 'bone' EVALUATE coherence RANK BY coherence DESC WITH CONFIDENCE, PROVENANCE",
      ...
      { q: "Which meningitis drugs reach inflamed CSF?",
        gql: "COVER ON mirador_universe WHERE disease = 'meningitis' AND tissue = 'csf_inflamed' EVALUATE coherence RANK BY coherence DESC WITH CONFIDENCE, PROVENANCE",
  ...
  },
  {
    ...
    questions: [
      ...
      { q: "Which drugs can beat drug-resistant TB?",
        gql: "COVER ON mirador_universe WHERE disease = 'tb' EVALUATE coherence RANK BY coherence DESC WITH CONFIDENCE, PROVENANCE",
  ...
  {
    label: 'BINDINGDB · 1.35M RECORDS', color: '#4ade80',
    questions: [
      { q: "Find the 50 most potent drug-target binding hits",
        gql: "COVER bindingdb_binding ON potency_class = 'potent' FIRST 50;",
        tag: "BindingDB · 1.35M", why: "Scanning 1.35M binding affinity records for potent hits — COVER returns 50 in <100ms without scanning every row" },
      { q: "Which organisms are most studied for drug binding?",
        gql: "COVER bindingdb_binding DISTINCT target_source_org;",
        tag: "organisms · distinct", why: "DISTINCT projects 1.35M binding records onto the organism fiber, revealing which species dominate drug-target research" },
      { q: "What potency classes exist in binding data?",
        gql: "COVER bindingdb_binding DISTINCT potency_class;",
        tag: "potency · classes", why: "Potency classes (potent, moderate, weak, inactive) partition the binding affinity fiber — one scan reveals the full distribution" },
  ...
  },
  {
    ...
    questions: [
      { q: "Find Phase 3 clinical trials",
        gql: "COVER clintrials_studies ON phase = 'PHASE3' FIRST 25;",
        tag: "Phase 3 · trials", why: "Phase 3 trials are the gold-standard for drug approval — filtering 578K studies by a single fiber coordinate returns the most advanced candidates instantly" },
      { q: "Browse interventional drug studies",
        gql: "COVER clintrials_studies ON study_type = 'INTERVENTIONAL' FIRST 25;",
        tag: "interventional", why: "Interventional trials test drug efficacy directly — bundle filtering bypasses ClinicalTrials.gov's slow search API" },
      { q: "Which trial phases exist?",
        gql: "COVER clintrials_studies DISTINCT phase;",
        tag: "phases · distinct", why: "DISTINCT collapses 578K studies onto the phase fiber — one scan reveals all trial phase categories" },
      { q: "What study types are in the database?",
        gql: "COVER clintrials_studies DISTINCT study_type;",
        tag: "study types", why: "Study type partitions the clinical trial fiber bundle — observational, interventional, expanded access, each a distinct geometric section" },
    ],
  },
  {
    label: 'PHARMACOGENOMICS · 14K RECORDS', color: '#c084fc',
    questions: [
      { q: "Which genes affect drug metabolism?",
        gql: "COVER pgx_variants DISTINCT gene;",
        tag: "PGx · genes", why: "Pharmacogenomic genes (CYP2D6, CYP2C19, etc.) determine how patients metabolize drugs — DISTINCT reveals which genes are catalogued" },
      { q: "CYP2D6 pharmacogenomic variants",
        gql: "COVER pgx_variants ON gene = 'CYP2D6' FIRST 25;",
        tag: "CYP2D6 · variants", why: "CYP2D6 metabolizes ~25% of all drugs. Poor metabolizers accumulate toxic levels, ultrarapid ones get nothing — see every known variant in one query" },
      { q: "Drug label testing recommendations",
        gql: "COVER pgx_drug_labels ALL FIRST 25;",
        tag: "labels · testing", why: "FDA/EMA drug labels increasingly require PGx testing — browse all pharmacogenomic label recommendations in a single fiber scan" },
      { q: "Clinical PGx annotations by evidence",
        gql: "COVER pgx_clinical ALL FIRST 25;",
        tag: "clinical · PGx", why: "PharmGKB curates clinical evidence for gene-drug interactions — each annotation is a validated pharmacogenomic fiber with evidence level" },
    ],
  },
  {
    ...
    questions: [
      { q: "Rank CNS drugs by AUC:MIC geometric potency index",
        gql: "COVER ON mirador_universe WHERE tissue = 'cns' EVALUATE coherence RANK BY coherence DESC WITH CONFIDENCE, PROVENANCE",
      ...
      { q: "Compare average binding τ by target organism",
        gql: "INTEGRATE bindingdb_binding OVER target_source_org MEASURE avg(tau), count(*);",
        tag: "BindingDB · organism", why: "τ = log₁₀(potency ratio) — INTEGRATE projects 1.35M binding records onto the organism fiber, producing a cross-species affinity ranking in one geometric operation" },
    ],
  },
  {
