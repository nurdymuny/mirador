...
abstention and error budgets.\ :contentReference[oaicite:0]{index=0}  HERALD instantiates this framework for viral
surveillance: it constructs an antigenic pullback manifold on sequence space and uses path-wise
drift statistics, Cantelli bounds, and a compositional error budget to bound dominance risk.\ :contentReference[oaicite:1]{index=1}
...
Euclidean distortion along those paths.:contentReference[oaicite:3]{index=3}
...
alerts).\ :contentReference[oaicite:4]{index=4}  MIRADOR must behave differently. When geometry or linkage is vacuous,
...
Davis manifolds supply the geometry and error budgets;\ :contentReference[oaicite:7]{index=7}  HERALD instantiates
geometry-first detection for antigenic drift;\ :contentReference[oaicite:8]{index=8}  MIRADOR adds a complementary,
...
\paragraph{Hit event and coverage.}
Given a candidate receptor panel $V \subset \Rcal$, MIRADOR defines the \emph{hit} event for a path $\Gamma$ as
\[
  \Hit(V,\Gamma)
  := \Bigl\{\exists\,t \in [0,1],\ \exists\,r \in V :
         \delta\bigl(\Gamma(t), r\bigr) \le \rho_{\mathrm{bind}} \Bigr\},
\]
i.e., the path passes within the binding radius of at least one receptor in $V$ at some time along its evolution.

The (true) coverage of $V$ under the forecast distribution is then
\[
  C(V) := \Pr_{\Gamma \sim \Pi_A(L)}\bigl(\Hit(V,\Gamma)\bigr),
\]
a number in $[0,1]$ once $\Pi_A(L)$ and $V$ are fixed. In later sections we will distinguish $C(V)$ from empirical estimates $\widehat{C}(V)$ obtained by sampling a finite number of drift paths from $\Pi_A(L)$ and discretizing them into representative antigenic states.
